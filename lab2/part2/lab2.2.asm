...
PM_seg	SEGMENT PARA PUBLIC 'CODE' USE32 ; указываем, что команды работают со всеми 32 битов регистров; например LOOP будет
    ...
    ; Таблица дескрипторов сегметов GDT
  	...
  	gdt_null	descr <>
        ...
        ; 16-битный 1Мб сегмент данных с базой = 0
        gdt_flatDS 	descr <0FFFFh, 0, 0, 10010010b, 00001111b, 0> ; 92h = 10010010b
...
my_print_eax macro
	local prcyc1 			; указываем, что метка локальная для макроса
		...
		add ebp,0B8010h 	; сейчас в EBP должно лежать положение первого символа на экране, 
					; с которого и будет распечатано число
...
new_int08:
	...
	push ebp
	mov ebp, 0	; (10 символов - 1 байт символа и 1 байт цвета )
...
end_of_memory:
	...
	push ebp
	mov ebp,20		; указываем смещение в видеопамяти относительно начала экрана 
